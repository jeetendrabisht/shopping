...
class Cart extends React.PureComponent {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (!!registerStatus && registerStatus.length) {
            ...
            if (userType !== "registered") {
                ...
                if (!!dbData) {
                    if (dbData.cart) {
                        dbData = JSON.parse(localStorage.getItem(sessionKey)).cart;
                        cartQuantity = dbData.reduce((acc, data) => {
                            return acc + data.quantity;
                        },0);
                        cartTotal = dbData.reduce((acc, data) => {
        ...
        return { cartData: dbData, cartQuantity: cartQuantity, cartTotal: cartTotal };
    }
    ...
    render() {
        const { cartData, cartQuantity } = this.state;
        return <div>
            <div>
