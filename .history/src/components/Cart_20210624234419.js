...
class Cart extends React.PureComponent {
    ...
    closeCartNoItem() {
        this.setState({noItemFlagRedirect: true});
        this.props.closeCart();
    }
    ...
    increment(data) {
        ...
        this.props.cartItem(cartQuantity);
        this.setState({
            cartData: cart,
    ...
    render() {
        const { cartData, cartQuantity, cartTotal, noItemFlagRedirect } = this.state;
        console.log("cartData ", cartData);
        console.log("cartQuantity ", cartQuantity);
        console.log("cartTotal ", cartTotal);
        return <div className="content-cart">
            <div className="layer-1">
                <div className="cart-header"><h3>My Cart {(cartData && cartData.length) ? <>&#40;{cartQuantity} item&#41;</> : null}</h3></div>
            ...
            <div className="layer-2">
                <div className="cartItem">
                    ...
                    }) : <div className="layer-2-noLength">
                        <center><p>No items in your cart</p>
                            <p>Your favorite items are just a click away</p></center>
                    </div>}
                </div>
                {(cartData && cartData.length) ? <div className="below-cartItems-tag">
                    <img src={"../../static/images/lowest-price.png"} alt="lowest_price_image" />
                </div> : null}
            </div>
            {(cartData && cartData.length) ? <div className="layer-3">
                <p>Promo code can be applied on payment page</p>
                <button type="button" className="cart-button">
                    <span>Proceed to Checkout</span>
                    <span>Rs.{cartTotal}&nbsp;&nbsp;&nbsp;&#62;</span>
                </button>
            </div> : <div className="layer-3-noLength">
                <button type="button" className="cart-button">
                    <span onClick={() => this.closeCartNoItem()}>Start Shopping
                    {noItemFlagRedirect && <Redirect to='/products' />}
                    </span>
                </button>
            </div>}
