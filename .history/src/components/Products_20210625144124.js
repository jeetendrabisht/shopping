...
class Products extends React.PureComponent {
    ...
    render() {
        ...
        return <div className="main-content-product">
            {screenWidth <= 600 ? <div>
                <select>
                <option value="All" onChange={() => this.showAll()}>All</option>
                    {category.map(categoryData => {
                        return <option key={categoryData.id} value={categoryData.name} onChange={() => this.onClickCategory(categoryData.id)}>{categoryData.name}</option>
                    })}
                </select>
