...
class Products extends React.PureComponent {
    ...
    render() {
        ...
        return <div className="main-content-product">
            {screenWidth <= 600 ? <div>
                <select onChange={(event) => this.onDropDownClick(event)}>
                <option key="choose" value="choose">Select Category</option>
                    {category.map(categoryData => {
                        return <option key={categoryData.id} value={categoryData.name}>{categoryData.name}</option>
                    })}
